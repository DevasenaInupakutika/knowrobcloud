...
///
/// Assumes all grids lie on the xy plane, and will fail if that's not true
nav_msgs::OccupancyGrid::Ptr mergeGrids(const std::vector<nav_msgs::OccupancyGrid::ConstPtr>& grids, double resolution);

/// Version of mergeGrids that uses the resolution of the first grid.
