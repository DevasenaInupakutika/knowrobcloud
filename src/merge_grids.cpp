...
{
  // The expansion is to avoid effects due to rounding when intersecting parallel grids
...
// Get the dimensions of a merged grid
nm::MapMetaData getMergedGridInfo (const vector<GridConstPtr>& grids, const double resolution)
{
...
// Main function
GridPtr mergeGrids (const vector<GridConstPtr>& grids, const double resolution)
{
  ...
  merged_grid->info.origin.position.x = -20;
  merged_grid->info.origin.position.y = -20;
      ...
      for (coord_t y=0; y<(int)grid->info.height; y++) {
        ...
        if ((value>=0) && (value<=100)) {
          BOOST_FOREACH (const Cell& intersecting_cell, 
                         intersectingCells(merged_grid->info, grid->info, cell)) {
            const index_t ind = cellIndex(merged_grid->info, intersecting_cell);
            merged_grid->data[ind] = max(merged_grid->data[ind], value);
          }
        }
      }
    }
  ...
  ROS_DEBUG_NAMED ("merge_grids", "Done merging grids");
  return merged_grid;
}


GridPtr mergeGrids (const vector<GridConstPtr>& grids)
{
  ROS_ASSERT (grids.size()>0);
  return mergeGrids(grids, grids[0]->info.resolution);
}
}
